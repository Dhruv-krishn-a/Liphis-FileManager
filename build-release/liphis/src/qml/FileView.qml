...
Item {
    ...
    PinchHandler {
        ...
        onActiveChanged: if (active) startZoom = zoomLevel
        onScaleChanged: {
            var newZoom = Math.max(50, Math.min(300, startZoom * scale))
            if (Math.abs(newZoom - zoomLevel) > 5) {
                zoomLevel = Math.round(newZoom / 5) * 5
            }
        }
    }
    ...
    Keys.onPressed: (event) => {
        console.log("[DEBUG] FileView Keys.onPressed. Key:", event.key, "Mod:", event.modifiers)
        if (!controller) return
    ...
    Component {
        ...
        Item {
            ...
            MouseArea {
                ...
                onClicked: (mouse) => {
                    console.log("[DEBUG] Item clicked:", model.path)
    ...
    StackLayout {
        ...
        ListView {
            ...
            Keys.onEscapePressed: (event) => {
                console.log("[DEBUG] ListView Keys.onEscapePressed caught Escape")
                if (controller) {
                    controller.clearSelection()
                    event.accepted = true
                }
        ...
        GridView {
            ...
            focus: true
            Keys.onEscapePressed: (event) => {
                console.log("[DEBUG] GridView Keys.onEscapePressed caught Escape")
                if (controller) {
                    controller.clearSelection()
                    event.accepted = true
                }
